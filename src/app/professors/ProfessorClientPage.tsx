...
export default function ProfessorClientPage({ professors }: { professors: Professor[] }) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-6">
        {professors.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
             <p className="text-xl text-gray-400 font-medium">No mentors added currently.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {professors.map((prof, i) => (
               ...
               >
                <div className="bg-white rounded-[2rem] border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] p-8 text-center transition-all duration-300 group flex flex-col w-full h-full">
                  <div className="relative w-36 h-36 mx-auto mb-6 rounded-full overflow-hidden border-8 border-gray-50 bg-gray-100 shadow-inner group-hover:scale-105 transition-transform duration-500 flex items-center justify-center shrink-0">
                    {prof.imageUrl ? (
                       <Image src={prof.imageUrl} alt={prof.name} fill className="object-cover" />
                    ) : (
                       <span className="text-4xl text-gray-300 font-bold">{prof.name.charAt(0)}</span>
                    )}
                  </div>
                  <h3 className="text-2xl font-black text-gray-900 mb-1 tracking-tight">{prof.name}</h3>
                  <p className="text-indigo-600 font-bold text-sm mb-1 uppercase tracking-wide">{prof.role}</p>
                  {prof.university && (
                    <p className="text-gray-500 font-medium text-sm mb-5">{prof.university}</p>
                  )}
                  ...
                  <div className="flex justify-center space-x-6 mb-8 px-4 py-4 bg-gray-50 rounded-2xl border border-gray-100 shrink-0">
                    ...
                    <div className="flex items-center text-sm font-bold">
                      {prof.acceptingMentees ? (
                        <span className="text-emerald-600 flex items-center">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 mr-2 animate-pulse"></span> Accepting
                        </span>
                      ) : (
