...
export default async function BlogPostPage({ params }: Props) {
  ...
  const date = format(new Date(post.publishedAt ?? post.createdAt), "MMMM d, yyyy");
    ...
    <div className="bg-[#FAFAFA] min-h-screen pt-32 pb-32">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500 mb-6">
          ...
          <span className="inline-flex items-center gap-1"><Clock className="w-4 h-4" /> {readingMinutes(post.content)} min read</span>
        </div>
        ...
        <div className="relative w-full aspect-[16/9] mb-10 rounded-3xl overflow-hidden shadow-sm border border-gray-100 bg-gray-100">
          {post.imageUrl ? (
            ...
            <EditorialCover category={post.category} size="hero" />
          )}
        </div>
          ...
          {post.externalLink && (
            <div className="mx-auto max-w-[44rem] mt-12 pt-6 border-t border-gray-100 text-sm text-gray-500">
              <a href={post.externalLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 hover:text-gray-900 underline-offset-4 hover:underline">
                {naverSource ? t.koreanOriginal : "Related publication"} <ExternalLink className="w-4 h-4" />
              </a>
            </div>
          )}
        </article>
        ...
        <div className="mt-10 rounded-3xl bg-gray-900 text-white p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <p className="text-2xl font-bold">Have a question you want to investigate?</p>
            <p className="text-gray-300 mt-2">Explore our faculty-guided programs, or tell admissions what you are curious about.</p>
          </div>
        ...
        {related.length > 0 && (
          <section className="mt-20" aria-labelledby="related-heading">
            <h2 id="related-heading" className="text-2xl font-black tracking-tight text-gray-900 mb-6">More in {post.category}</h2>
                ...
                <Link key={item.id} href={`/blog/${item.slug || item.id}`} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-all flex flex-col">
                  <div className="relative h-36 bg-gray-100 overflow-hidden">
                    {item.imageUrl ? (
                      ...
                      <EditorialCover category={item.category} />
                    )}
                  </div>
