...
const schema = z.object({
  ...
  /** Browser-generated id shared with the pixel's Lead event so Meta counts the inquiry once. */
  eventId: z.string().max(64).optional(),
});
export type ContactResult = { success: true; tracking: MetaCustomData } | { success: false; error: string };
export async function submitContactForm(input: z.input<typeof schema>): Promise<ContactResult> {
  ...
  try {
    ...
    const program = data.programId ? await prisma.program.findFirst({ where: { id: data.programId, isPublished: true }, select: { id: true, title: true, category: true, subCategory: true, professors: { take: 1, select: { name: true, university: true, relatedMajor: true, potentialTopics: true } } } }) : null;
    ...
    // Conversions API copy of the Lead event (hashed email/name/country only); the browser fires the same event id.
    const { data: tracking } = await sendMetaEvent({
      name: 'Lead', eventId: safeEventId(data.eventId),
