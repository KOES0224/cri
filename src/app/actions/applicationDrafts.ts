...
export async function saveApplicationDraft(programId: string, input: unknown, step: number, expectedVersion: number) {
  ...
  if (!session?.user?.id || !canApply(session.user.role)) return { success: false as const, error: 'Sign in with your student or parent account to save.', code: 'draftSignIn' };
  const parsed = applicationDraftSchema.safeParse(input);
  if (!parsed.success || !Number.isInteger(step) || step < 1 || step > 3 || !Number.isInteger(expectedVersion) || expectedVersion < 0) return { success: false as const, error: 'The draft could not be saved. Check your entries.', code: 'draftInvalid' };
  try {
    ...
    if (parsed.data.resumeUrl) {
      ...
      if (!document) return { success: false as const, error: 'Upload the resume using this account.', code: 'draftResumeAccount' };
    }
    ...
    if (expectedVersion === 0) {
      ...
      if (result.count !== 1) return { success: false as const, conflict: true, error: 'Another tab created a draft. Copy your recent edits, then reload.', code: 'draftConflictCreated' };
    ...
    if (result.count !== 1) return { success: false as const, conflict: true, error: 'Another tab changed this draft. Copy your recent edits, then reload to continue.', code: 'draftConflictChanged' };
    return { success: true as const, version, savedAt: savedAt.toISOString() };
  } catch (error) {
    const conflict = (error as { code?: string }).code === 'P2002';
    return { success: false as const, conflict, error: conflict ? 'Another tab created a draft. Copy your recent edits, then reload.' : 'Draft not saved. Check your connection and retry before leaving.', code: conflict ? 'draftConflictCreated' : 'draftFailed' };
  }
}
