...
export default function LeadHeaderActions({ leadId, linkedUser, createdBy }: { leadId: string, linkedUser: any, createdBy: string | null }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm("Are you sure you want to permanently delete this inquiry and its history? This cannot be undone.")) return;
    ...
    if (res.success) {
      ...
      alert("Could not delete inquiry: " + res.error);
      setIsDeleting(false);
    }
  };

  const handleLinkUser = async () => {
    if (!linkInput.trim() || isSubmittingLink) return;
    setIsSubmittingLink(true);
    setLinkError("");

    try {
      const res = await manuallyLinkLeadToUser(leadId, linkInput);
      if (!res.success) throw new Error(res.error || "Could not link this person.");
      setIsLinking(false);
      setLinkInput("");
      router.refresh();
    } catch (error) {
      setLinkError(error instanceof Error ? error.message : "Could not link this person. Please retry.");
        ...
        <div className="relative">
          ...
          {isLinking && (
            <div className="absolute right-0 sm:right-auto sm:left-0 top-full mt-2 w-72 bg-white rounded-xl shadow-lg border border-gray-200 p-4 z-50 animate-in fade-in slide-in-from-top-2">
               <h4 className="text-sm font-bold text-gray-900 mb-2">Link to person</h4>
               <p className="text-xs text-gray-500 mb-3">Enter the user's registered Email or Student Code.</p>
               <input
                 autoFocus
                 type="text"
                 aria-label="Registered email or student code"
                 placeholder="Email or student code"
                 value={linkInput}
